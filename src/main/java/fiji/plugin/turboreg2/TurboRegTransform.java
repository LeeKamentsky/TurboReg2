...
....................................................................*/
     ...
     * of accuracy. This number must be corrected so that there are more iterations at the coarse
    ...
    /**
     * *******************************************************************
     * Maximal number of registration iterations per level, when accuracy is requested at the
     * expense of speed. This number must be corrected so that there are more iterations at the
     ...
     * @author Lee Kamentsky
     *
     * An image / mask pair, returned together.
     */
    static class ImageAndMask {

        /**
         * The image after transformation
         */
        final public ImgPlus<FloatType> image;
        /**
         * The mask after transformation with 1.0 being unmasked and 0.0 being masked and values
         * in-between representing output pixels straddling masked and unmasked input pixels.
         */
        final public ImgPlus<FloatType> mask;
        final float[] image_pixels;
        final float[] mask_pixels;

        /**
         * Construct an image and mask of the given height and width
         *
         * @param width
         * @param height
         */
        public ImageAndMask(int width, int height) {
            image_pixels = new float[width * height];
            mask_pixels = new float[width * height];
            image = ImgPlus.wrap(ArrayImgs.floats(image_pixels, width, height));
            mask = ImgPlus.wrap(ArrayImgs.floats(mask_pixels, width, height));
        }
    }
    ...
    public ImageAndMask doFinalTransform() {
        ...
        inNx = sourceImg.getWidth();
        inNy = sourceImg.getHeight();
        final ImageAndMask result = new ImageAndMask(targetImg.width, targetImg.height);
        twiceInNx = 2 * inNx;
        twiceInNy = 2 * inNy;
        outImg = result.image_pixels;
        final double[][] matrix = getTransformationMatrix(targetPoint, sourcePoint);
        switch (transformation) {
            case TRANSLATION: {
                translationTransform(matrix, result.mask_pixels);
                break;
            }
            case RIGID_BODY:
            case SCALED_ROTATION:
            case AFFINE: {
                affineTransform(matrix, result.mask_pixels);
                break;
            }
            case BILINEAR: {
                bilinearTransform(matrix, result.mask_pixels);
                break;
            }
        }
        return result;
    ...
    public void doRegistration() {
        ...
        while (!targetImgPyramid.isEmpty()) {
            ...
            if (transformation == TransformationType.BILINEAR) {
        ...
        twiceInNy = 2 * inNy;
        if (accelerated) {
            skipProgressBar(iterationCost * (maxIterations - 1));
        } else {
            switch (transformation) {
                case RIGID_BODY: {
                    inverseMarquardtLevenbergRigidBodyOptimization(
...
....................................................................*/
 ...
 /*------------------------------------------------------------------*/
    private void affineTransform(
        ...
        int yMsk;
        int k = 0;
        addWorkload("Applying affine transform", outNy);
        yx = matrix[0][0];
        yy = matrix[1][0];
 ...
 /*------------------------------------------------------------------*/
    private void bilinearTransform(
        ...
        int yMsk;
        int k = 0;
        addWorkload("Applying bilinear transform", outNy);
        yx = matrix[0][0];
        yy = matrix[1][0];
        yxy = 0.0;
        yyy = 0.0;
 ...
 /*------------------------------------------------------------------*/
    private double getAffineMeanSquares(
        ...
        uv21 /= det;
        uv13 /= det;
        for (int i = 0; (i < gradient.length) && (i < hessian.length); i++) {
            gradient[i] = 0.0;
        ...
        for (int i = 1; (i < hessian.length) && (i < hessian[0].length); i++) {
            for (int j = 0; (j < i); j++) {
 ...
 /*------------------------------------------------------------------*/
    private void inverseMarquardtLevenbergOptimization(
        ...
        final int nPoints = TurboRegPointHandler.pointCount(transformation);
        final double[][] attempt = new double[nPoints][2];
        final double[][] hessian = new double[nPoints * 2][nPoints * 2];
        final double[][] pseudoHessian = new double[nPoints * 2][nPoints * 2];
        final double[] gradient = new double[nPoints * 2];
        double[][] matrix = getTransformationMatrix(sourcePoint, targetPoint);
        double[] update = new double[nPoints * 2];
        ...
        switch (transformation) {
            ...
            default: {
                throw new Error("Not applicable for " + transformation.getDisplayName() + " transform.");
            }
        }
        iteration++;
        do {
            for (int k = 0; (k < pseudoHessian.length); k++) {
        ...
        skipProgressBar(workload * iterationCost);
    }
 ...
 /*------------------------------------------------------------------*/
    private void inverseMarquardtLevenbergRigidBodyOptimization(
        ...
        final double[][] pseudoHessian = new double[3][3];
        ...
        double displacement;
        int iteration = 0;
        for (int k = 0; (k < sourcePoint.length); k++) {
 ...
 /*------------------------------------------------------------------*/
    private void MarquardtLevenbergOptimization(
        ...
        final int nPoints = TurboRegPointHandler.pointCount(transformation);
        final int vectorSize = nPoints * 2;
        final double[][] attempt = new double[nPoints][2];
        final double[][] hessian = new double[vectorSize][vectorSize];
        final double[][] pseudoHessian = new double[vectorSize][vectorSize];
        final double[] gradient = new double[vectorSize];
        double[][] matrix = getTransformationMatrix(targetPoint, sourcePoint);
        double[] update = new double[vectorSize];
        double bestMeanSquares = 0.0;
        double meanSquares = 0.0;
        double lambda = FIRST_LAMBDA;
        double displacement;
 ...
 /*------------------------------------------------------------------*/
    private void scaleBottomDownLandmarks() {
        for (int depth = 1; (depth < pyramidDepth); depth++) {
            for (int n = 0; n < TurboRegPointHandler.pointCount(transformation); n++) {
                sourcePoint[n][0] *= 0.5;
                sourcePoint[n][1] *= 0.5;
                targetPoint[n][0] *= 0.5;
                targetPoint[n][1] *= 0.5;
 ...
 /*------------------------------------------------------------------*/
    ...
    protected void skipProgressBar(int increment) {
        progress += increment;
        if (statusService != null) {
            statusService.showProgress(progress, workload);
        }
