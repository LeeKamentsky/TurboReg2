 ...
 * registration done by <code>TurboRegRegister</code>
 * to an image.
 */
...
public class TurboRegApply extends TurboRegCommandBase {
	...
	@Override
	protected TableDisplay getTableDisplay() { return registrationDisplay; }
	@Override
	...
	public void run() {
		...
		ArrayList<Future<?>> runningPlanes = new ArrayList<Future<?>>();
		if (plane.length == 0) {
			runningPlanes.add(runPlane(plane));
		} else if (allFrames) {
			int nPlanes = 1;
			for (int i=0; i<plane.length; i++) nPlanes *= display.dimension(i+2);
			...
			for (int i=0; i<plane.length; i++) plane[i] = view.getLongPosition(i+2);
			int channelAxis = display.getAxisIndex(Axes.CHANNEL);
			if (channelAxis < 0) {
				runningPlanes.add(runPlane(plane));
			} else {
	...
	/**
	 * The <code>RunPlane</code> class is a runnable
	 * that transforms one plane of the display. 
	 */
		...
		public void run() {
			try {
				...
				for (int d=data.numDimensions()-1; d >=2; d--) {
					if (data.axis(d).equals(Axes.CHANNEL)) channel = new Integer(d);
